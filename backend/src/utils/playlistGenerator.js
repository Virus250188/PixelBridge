 ...
 * IMPORTANT: These names must match EXACTLY what RetroArch uses for save/state directories!
 * RetroArch uses the core's display name, which is typically capitalized.
 * @param {string} platformShortName - Platform short name (e.g., 'snes')
 * @returns {string}
 */
function getCoreName(platformShortName) {
  // These MUST match the actual directory names RetroArch creates in /saves/ and /states/
  // Verified from Apple TV: /saves/Nestopia/, /states/Nestopia/
  const mapping = {
    'snes': 'Snes9x',
    'nes': 'Nestopia',
